...
class LLMJudge:
    ...
    def score_response(
        ...
        """
        prompt = (
            "You are an expert AI evaluation judge. Evaluate the following response based on the provided rubric.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            "Rubric:\n"
        )
        ...
        prompt += (
            "\nReturn your evaluation as a valid JSON object with the criteria scores between 0.0 and 1.0 (or 1 to 5) and reasoning.\n"
            'Example format: {"scores": {"accuracy": 0.8}, "reasoning": "Well explained"}\n'
        )
        ...
        try:
            ...
            parsed = json.loads(match.group(0) if match else str(raw_response))
            if isinstance(parsed, dict):
                if "scores" in parsed and isinstance(parsed["scores"], dict):
                    scores = {str(k): float(v) for k, v in parsed["scores"].items() if isinstance(v, (int, float, str))}
        ...
        for item in scores_batch:
            if isinstance(item, dict):
                ...
                if isinstance(scores_dict, dict):
                    ...
                    if item_vals:
                        item_averages.append(sum(item_vals) / len(item_vals))
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_avg_f = (sum(r.faithfulness for r in new_results) / new_len) if new_len else 0.0
        new_avg_r = (sum(r.relevance for r in new_results) / new_len) if new_len else 0.0
        new_avg_c = (sum(r.completeness for r in new_results) / new_len) if new_len else 0.0

        base_avg_f = (sum(r.faithfulness for r in baseline_results) / base_len) if base_len else 0.0
        base_avg_r = (sum(r.relevance for r in baseline_results) / base_len) if base_len else 0.0
        base_avg_c = (sum(r.completeness for r in baseline_results) / base_len) if base_len else 0.0
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, f in enumerate(failures, start=1):
            fid = f"F{idx:03d}"
            ftype = str(getattr(f, "failure_type", "") or "Unknown")
    ...
    def generate_improvement_suggestions(
        ...
        if any(k.lower() in ("hallucination", "faithfulness") for k in categories) or any(f.faithfulness < 0.5 for f in failures):
            suggestions.append("Implement hallucination checker to filter unsupported claims")
            suggestions.append("Improve prompt grounding and strict context-only instructions")

        if any(k.lower() in ("irrelevant", "off_topic", "relevance") for k in categories) or any(f.relevance < 0.5 for f in failures):
            suggestions.append("Refine system prompt and intent routing to keep answers on topic")
            suggestions.append("Improve query classification and prompt clarity")

        if any(k.lower() in ("incomplete", "completeness", "low_completeness") for k in categories) or any(f.completeness < 0.5 for f in failures):
            suggestions.append("Increase chunk size and top-k in RAG pipeline to reduce context fragmentation")
        ...
        default_suggestions = [
            ...
            "Add few-shot examples showing complete answers to improve completeness",
            "Implement hallucination checker to filter unsupported claims",
            "Refine system prompt and intent routing to keep answers on topic",
        ]
